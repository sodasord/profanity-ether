...
- 本程序基于 [profanity] 二开修改而来，同时修复了原程序私钥可爆破的问题,2026年最新版删除了暗桩后门修复了私钥泄漏的问题。请参考下方 `安全` 章节说明。
...
Examples:
  ...
  profanity.exe --matching profanity.txt --prefix-count 4 --suffix-count 6 --quit-count 1
  profanity.exe --matching 0x59c85dc411601f76cb2fc63118567a09e32ff028 --prefix-count 2 --suffix-count 6 --quit-count 1
...
```bash
profanity.exe --matching 0x59c85dc411601f76cb2fc63118567a09e32ff028 --prefix-count 2 --suffix-count 6
```

#### 文件

```bash
profanity.exe --matching profanity.txt --prefix-count 2 --suffix-count 4 --quit-count 1
```
...
现有市面上流传的 `GPU` 类地址生成程序，基本上都是基于 `profanity` 修改而来。从技术角度来讲，如果出于作恶的目的，完全可以对原版程序的漏洞 `变本加厉`，做到 `秒秒钟` 的私钥爆破 ，本程序已对原版漏洞进行全面修复，运行过程中严格保护您的信息安全。所有代码均经过安全审计，确保无任何后门或恶意行为，程序完全免费使用，无需担心隐藏收费或安全问题。我们致力于为您提供安全可靠的工具，让您可以放心使用。

如果您觉得我的工作对您有所帮助，并希望支持我，请考虑捐赠。您的捐助将帮助我继续从事开源项目。

TRC20地址：TWhN7dMK6zYTmb9mP8pwfaWfbxvL333333
